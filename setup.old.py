...
def setup():
    ...
    try:
        ...
        documents = reviews_df['Positive_Review'].tolist()
        # Select only the specific columns to store as metadata. (only positives reviews)
        metadata_columns = ['Review_Date', 'Positive_Review', 'Reviewer_Score']
        metadatas = reviews_df[metadata_columns].to_dict('records')
        vector_store = FAISS.from_texts(documents, embeddings, metadatas=metadatas)
